...
2. Injects clarification + self-improvement protocol pointers into every skill's `SKILL.md`
...
### `bash install-skillset.sh <git-url> [flags]`

Imports an external skill repository into this one.
...
### `bash update-skillsets.sh [flags]`

Re-pulls the latest version of every skillset tracked in `.skillsets.json`.
...
bash update-skillsets.sh --skip-setup
```
...
Regenerates the skills table in `README.md` from whatever skill folders exist at the repo root. Called automatically by `setup.sh` and `install-skillset.sh` — only run directly if you need to update the table without running the full setup.
...
│ follow the clarification protocol at: /abs/path/...      │  ← Injected by setup.sh
...
│ finish, read and follow the self-improvement protocol... │  ← Injected by setup.sh
...
- Do not manually edit the two managed protocol blocks — they are regenerated by `setup.sh`
...
Do not edit this file by hand. `install-skillset.sh` and `update-skillsets.sh` manage it.
