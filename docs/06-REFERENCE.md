...
| Frontmatter has only allowed keys (`name`, `description`, `license`, `compatibility`, `allowed-tools`, `session-memory`, `metadata`) | Error |
...
| Order is correct | `grep -n "BEGIN dev-agent-skills" fix-bug/SKILL.md` (clarification, then self-improvement, then session-memory if present) |
