...
## `fix-bug` fix-attempt ledger added — stop re-proposing rejected fixes

**Before:** A developer reports a bug; `fix-bug` proposes a fix and applies it; the issue persists; the developer says it's wrong; the skill reverts and proposes a new fix. If that second attempt *also* failed, asking once more could resurface the **first rejected fix**, because nothing tracked what had already been tried and turned down. Neither existing memory protocol covered this: `graph-memory` tracks whether a graph *query* was useful, `session-memory` avoids repeating investigation *within one conversation*. Neither has any concept of "this specific fix idea was already rejected."

**After:** A per-bug, append-only ledger at `.dev-agent/fix-attempts/<hash>.json` in the target repo (`fix-bug/scripts/ledger-lib.mjs`, `ledger-cli.mjs`). Before proposing a fix, Step 7 checks the hypothesis against every prior `rejected` attempt, matched by normalized substring *and* word-overlap ratio — a re-derived idea is rarely reworded in the same order ("app ID constant in config/links.ts was wrong" vs. "config/links.ts app ID was wrong" needed the second matching layer). A `rejected` record requires the developer's actual feedback text, not just a pass/fail flag — the one thing a diff can never contain. Outcomes are corrected via an explicit `supersedes` reference rather than editing the original record, so the ledger stays append-only even when a `pending` fix is later confirmed, or an already-`accepted` fix is reopened after being found broken in production. New Step 13 handles the "developer reports it didn't work" re-entry, which the skill previously had no path for at all — it only ever assumed success.

**Why:** The failure being designed against is a memory gap, not a reasoning gap — the model wasn't bad at debugging, it was re-deriving a hypothesis nothing on disk said was already tried. Not commit-based: Step 10's commit gate defaults to *not* committing, so most rejected attempts are never committed, and a ledger keyed to commit history would be blind to exactly the case it needs to cover.

**Two real bugs found during manual CLI testing, not by the unit tests alone**, both now regression-tested and in `fix-bug/references/edge-cases.md`: a double-supersede could corrupt the ledger (the validator checked the target's own `outcome` field, but append-only means that field never changes, so a superseded `pending` record stays `"pending"` forever — the fix scans for an existing resolver instead); and the fix for that then blocked the legitimate case of reopening an already-accepted fix later found broken, requiring the rule to distinguish "already resolved once" (blocked) from "resolving with a genuinely different outcome" (allowed — this is reopening).

---

## Shared circuit breaker added — bounds `sync-prs`'s unbounded CI-remediation loop

**Before:** `sync-prs` Step 7 read "repeat until green or blocked on something that needs a human" — no turn cap, no token cap, no memory of how many attempts had already been made on a given PR. This is the same shape as a documented real cost incident: a recursion loop that ran for hours because nobody defined an exit condition.

**After:** `scripts/circuit-breaker.mjs` — a **shared** primitive at the repo's top-level `scripts/`, not `sync-prs`-only, since `fix-bug`'s own retry loop needs the identical mechanism and three independently hand-rolled caps would drift apart. Three deliberate properties: pre-flight (`checkBreaker()` runs *before* the risky action, never after — by the time a post-flight check fires, the cost is already spent); raises rather than returns (`CircuitBreakerError` is a thrown exception, so a caller that ignores a return value cannot accidentally continue past a tripped breaker the way it could with a boolean); no grace period (turn N is fine, turn N+1 trips immediately, tested at both boundaries). The ledger records `input_hash`, never raw input text — the same principle `dev-agent`'s own `ledger.db` uses. Sessions are per-loop-instance (`pr-<N>-ci-remediation`), so one PR's remediation tripping never consumes budget a different PR's remediation needs.

`sync-prs/SKILL.md` Step 7 now checks the breaker before every remediation attempt and records the outcome after every re-check; a trip stops remediation on that PR only, reported via a new 🔴 CI-legend symbol distinct from an ordinary unresolved failure, without blocking any other PR in the same sync run.

**Why:** This is the exact failure mode the production-safety design work was written to prevent before any of this repo's skills existed — real documented cost incidents from agents retrying indefinitely because nobody defined an exit condition. `sync-prs` had run with precisely that unbounded shape until this closed it.

**One real bug found and fixed during test-writing, not shipped:** an early path-traversal test asserted the sanitized session-ID filename must never contain the literal substring `..`, which is the wrong property — the sanitizer strips path separators (what actually prevents traversal) but allows literal dots through for ordinary filenames, so `../../etc` correctly becomes a harmless single-segment filename that still contains `..` as inert text. Fixed to check the actual safety property (resolved path cannot escape `repoRoot`; result is always a single file, never a subdirectory) instead of a substring heuristic that would have been wrong in both directions.

---

## Current state snapshot (at time of writing)

21 skills at the repo root: `coding-standards` + its 7 domain sub-skills (including `coding-standards-tailwind`), `eslint-rule-author`, `first-principles-review`, `fix-bug`, `graphify`, `investigate-issue`, `plan-feature`, `review-pr`, `skill-add`, `skill-factory`, `skill-update`, `sync-prs`, `typescript-conventions`, `webapp-conventions`. Four managed protocols: clarification and self-improvement unconditional; session-memory opt-in (`coding-standards`, `sync-prs`, `review-pr`); graph-memory opt-in (`fix-bug`, `plan-feature`, `review-pr`). Six standing rules (0, 0b, 1, 2, 3, and the meta-principle) in `AGENT-STANDING-RULES.md`. `coding-standards`' domain manifest at version 4, broadened three times (database/frontend for TypeORM/Sequelize/Angular/Astro; backend/frontend for NestJS/Nuxt; a wholly new domain, Tailwind, added rather than an existing one broadened), its detection fed by `graphify`'s own build rather than a separate pass. `fix-bug` additionally checks blast radius via `graphify affected` before finalizing a fix, and maintains a per-bug fix-attempt ledger (`.dev-agent/fix-attempts/`, target-repo-local) so a rejected hypothesis is never re-proposed. `sync-prs` uses the shared circuit breaker at top-level `scripts/circuit-breaker.mjs` to bound its CI-remediation loop, keyed per PR (`pr-<N>-ci-remediation`) so one runaway PR never affects another's remediation in the same run; the same primitive is available to any future looping skill. `validate_skill.py` fully passes across all 21.
